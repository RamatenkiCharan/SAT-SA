# Historical SAT-SA P0 Requirements-to-Test Traceability Matrix

> This matrix is retained as a historical traceability snapshot. Its original
> aggregate counts and PASS labels are not current release evidence. The
> current data-quality formula is 0.35 completeness, 0.25 consistency, 0.25
> coverage, and 0.15 sample sufficiency; current verification evidence belongs
> in `current-state.md` and the latest test run.
